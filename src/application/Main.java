...
public class Main extends Application {
    ...
    public void start(final Stage stage) throws Exception {
        ...
        quoteButton.setGraphic(new ImageView(new Image(
                "http://img.freebase.com/api/trans/image_thumb/en/benjamin_franklin?pad=1&errorid=%2Ffreebase%2Fno_image_png&maxheight=64&mode=fillcropmid&maxwidth=64")));
        ...
        controls.getChildren().addAll(quoteButton, new Separator(), widthSlider, heightSlider, layoutXSlider, layoutYSlider, new Separator(),
                quotedText);
        ...
        selectedQuote.addListener(new ChangeListener<Label>() {
            @Override
            public void changed(ObservableValue<? extends Label> observableValue, Label oldQuote, final Label newQuote) {
                if (oldQuote != null) {
    ...
    class MessageBoard extends Pane {
        ...
        Label post(String quote, Color color) {
            ...
            String bgColor = "#" + color.deriveColor(color.getHue(), color.getSaturation(), color.getBrightness(), random.nextDouble() * 0.5 + 0.5)
                    .toString().substring(2, 10);
            label.setStyle("-fx-background-radius: 5; -fx-background-color: linear-gradient(to bottom, " + bgColor + ", derive(" + bgColor
                    + ", 20%)); -fx-text-fill: ladder(" + bgColor
                    + ", lavender 49%, midnightblue 50%); -fx-font: 18px 'Segoe Script'; -fx-padding:10;");
            ...
            label.relocate(random.nextInt((int) Math.floor(this.getPrefWidth() - label.getPrefWidth())),
    ...
    class Quotes {
        private final String[] quotes = {"A house is not a home unless it contains food and fire for the mind as well as the body.",
            "A life of leisure and a life of laziness are two things. There will be sleeping enough in the grave.",
            "A penny saved is a penny earned.", "A place for everything, everything in its place.", "Admiration is the daughter of ignorance.",
            "All mankind is divided into three classes: those that are immovable, those that are movable, and those that move.",
            "All wars are follies, very expensive and very mischievous ones.", "An investment in knowledge pays the best interest.",
            "Anger is never without a reason, but seldom with a good one.", "Any fool can criticize, condemn and complain and most fools do.",
    ...
    class Colors {
        final String[][] smallPalette = {{"aliceblue", "#f0f8ff"}, {"antiquewhite", "#faebd7"}, {"aqua", "#00ffff"}, {"aquamarine", "#7fffd4"},
